...
// 发送数据包函数，给目的短地址发送指定长度的数据，发送成功刚返回SUCCESS，失败则返回FAILED
// 参数分别为目的短地址、指向发送缓冲区的指针、发送数据长度
uint8 basicRfSendPacket(uint16 destAddr, uint8* pPayload, uint8 length);

// 检查是否收到一个新数据包，确定模块是否已经可以接收下一个数据，如果准备好则返回TRUE
uint8 basicRfPacketIsReady(void);             
int8 basicRfGetRssi(void);

// 接收来自Basic RF层的数据包，并为所接收的数据和RSSI值配缓冲区
